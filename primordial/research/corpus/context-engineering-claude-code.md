# source: https://www.claudedirectory.org/blog/context-engineering-claude-code
...
1. **`./CLAUDE.md`** — Project root (applies to this repo)

1. **`./src/CLAUDE.md`** — Directory-specific (applies when working in that directory)
...
MCP ServerContext It Provides**Database MCP**Real schema, sample data, relationships**Figma MCP**Actual design specs, not verbal descriptions**GitHub MCP**Issues, PRs, review comments**Sentry MCP**Real error traces, stack traces, frequency data**Browser MCP**What the page actually looks like, console errors
...
1. **Month 1**: Your CLAUDE.md is refined from real sessions. Memory captures your preferences. Claude feels like it knows your project.

1. **Month 3**: Directory-level CLAUDE.md files guide behavior in different parts of the codebase. MCP servers provide live data. Claude produces code that passes your review on the first try most of the time.
