# source: https://www.claudedirectory.org/blog/claude-code-auto-memory-guide
...
1. **Confirm good calls, not just bad ones.** Say "yes, that's right" when Claude makes a non-obvious judgment you agree with.

1. **Explain the *why* when you correct.** "Don't use mocks in DB tests — we had an incident last quarter where mocks passed but prod failed" is a better memory than just "don't use mocks."

1. **Commit project memories with your repo.** If a memory is about the project and not about you personally, check it in so your teammates inherit it.

1. **Treat stale memories as staleness, not errors.** Project memories decay fast. If Claude cites an old memory, it's not lying — it just saved a snapshot. Update it and move on.
